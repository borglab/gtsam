...
TEST(NonlinearINequalityConstraint, TrivialInequality1) {
  ...
  Values testX;
  testX.insert(X, 2.0);
  CHECK(0.0 == inequality.unwhitenedError(testX)[0]);
  CHECK(-29 == inequality.equality_.unwhitenedError(testX)[0]);
}
...
TEST(NonlinearInequalityConstraint, TrivialInequality2) {
  ...
  testX.insert(X2, 0.0);
  CHECK(0.0 == inequality.unwhitenedError(testX)[0]);
  CHECK(0.0 == inequality.equality_.unwhitenedError(testX)[0]);
}
...
typedef NonlinearInequalityConstraint3<double, double, double,
    TrivialInequalityError3> TrivialInequality3;
...
TEST(NonlinearInequalityConstraint, TrivialInequality3) {
  ...
  testX.insert(X2, 4.0);
  testX.insert(X3, 9.0);
  CHECK(666 == inequality.unwhitenedError(testX)[0]);
  CHECK(666 == inequality.equality_.unwhitenedError(testX)[0]);
}
