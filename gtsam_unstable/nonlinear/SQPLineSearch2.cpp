...
/* ************************************************************************* */
std::pair<NonlinearFactorGraph, NonlinearFactorGraph> SQPLineSearch2::split(
    const NonlinearFactorGraph& graph) const {
  NonlinearFactorGraph unconstrained, constrained;
  for(const NonlinearFactor::shared_ptr factor: graph){
    NonlinearEqualityConstraint::shared_ptr factorC = toConstraint(factor);
    if (factorC) {
      constrained.push_back(factor);
    }
    else {
...
/* ************************************************************************* */
MeritFunction2::MeritFunction2(const NonlinearFactorGraph& unconstrained,
  ...
  unconstrained_(unconstrained), constrained_(constrained), linearUnconstrained_(
