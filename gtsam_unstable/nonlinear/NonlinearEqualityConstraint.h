...
public:
  ...
  NonlinearEqualityConstraint1(Key key, Key dualKey, size_t constraintDim = 1) :
    Base(noiseModel::Constrained::All(constraintDim), key), NonlinearConstraint(
    dualKey) {
  }
   ...
   */
  virtual Vector
  evaluateError(const X& x, boost::optional<Matrix&> H1 = boost::none) const override {
    ERROR_FUNCTOR f;
    return f(x, H1);
  }
...
public:
  ...
  NonlinearEqualityConstraint2(Key j1, Key j2, Key dualKey,
    ...
    Base(noiseModel::Constrained::All(constraintDim), j1, j2), NonlinearConstraint(
    dualKey) {
  }
...
class NonlinearEqualityConstraint3: public NoiseModelFactor3<VALUE1, VALUE2,
  VALUE3>, public NonlinearConstraint {
...
public:
  ...
  NonlinearEqualityConstraint3(Key j1, Key j2, Key j3, Key dualKey,
                               size_t constraintDim = 1) :
    Base(noiseModel::Constrained::All(constraintDim), j1, j2, j3), NonlinearConstraint(
    dualKey) {
  }
   ...
   */
  virtual Vector
  evaluateError(const X1& x1, const X2& x2, const X3& x3, boost::optional<Matrix&> H1 =
  boost::none, boost::optional<Matrix&> H2 = boost::none,
                boost::optional<Matrix&> H3 = boost::none) const override {
    ERROR_FUNCTOR f;
    return f(x1, x2, x3, H1, H2, H3);
  };
