  ...
  public:
    ...
    public:
      ...
      void integrateMeasurement(
        ...
        // Update Jacobians
        delPdelBiasAcc += delVdelBiasAcc * deltaT - 0.5 * deltaRij.matrix() * body_R_sensor.matrix() * deltaT*deltaT;
        delPdelBiasOmega += delVdelBiasOmega * deltaT - 0.5 * deltaRij.matrix()
          * skewSymmetric(body_R_sensor.matrix() * biasHat.correctAccelerometer(measuredAcc)) * deltaT*deltaT * delRdelBiasOmega;
        delVdelBiasAcc += -deltaRij.matrix() * body_R_sensor.matrix() * deltaT;
        delVdelBiasOmega += -deltaRij.matrix() *
            skewSymmetric( body_R_sensor.matrix() * biasHat.correctAccelerometer(measuredAcc)) * deltaT * delRdelBiasOmega;
        ...
        delRdelBiasOmega = ( body_R_sensor.matrix() * Rincr.inverse().matrix() * body_R_sensor.inverse().matrix() * delRdelBiasOmega )
             - body_R_sensor.matrix() * Jr  * deltaT;

        // Update preintegrated measurements
        deltaPij += deltaVij * deltaT
            + 0.5 * deltaRij.matrix() *  body_R_sensor.matrix() * biasHat.correctAccelerometer(measuredAcc) * deltaT*deltaT;
        deltaVij += deltaRij.matrix() * body_R_sensor.matrix() * biasHat.correctAccelerometer(measuredAcc) * deltaT;
        deltaRij = deltaRij * ( body_R_sensor * Rincr * body_R_sensor.inverse());
        deltaTij += deltaT;
      }
