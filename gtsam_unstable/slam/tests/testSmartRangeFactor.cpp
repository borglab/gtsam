...
TEST( SmartRangeFactor, unwhitenedError ) {
  ...
  EXPECT(assert_equal(Vector{{0.0}}, actual1));
  f.addRange(2, r2);
  Vector actual2 = f.unwhitenedError(values);
  EXPECT(assert_equal(Vector{{0.0}}, actual2));

  f.addRange(3, r3);
  vector<Matrix> H(3);
  Vector actual3 = f.unwhitenedError(values);
  EXPECT_LONGS_EQUAL(3, f.keys().size());
  EXPECT(assert_equal(Vector{{0.0}}, actual3));

  // Check keys and Jacobian
  Vector actual4 = f.unwhitenedError(values, H); // with H now !
  EXPECT(assert_equal(Vector{{0.0}}, actual4));
  CHECK(assert_equal(Matrix{{0.0, -1.0, 0.0}}, H.front()));
  CHECK(assert_equal(Matrix{{sqrt(2.0) / 2, -sqrt(2.0) / 2, 0.0}}, H.back()));
...
TEST( SmartRangeFactor, optimization ) {
  ...
  EXPECT(assert_equal(Vector{{sqrt(25.0 + 16.0) - sqrt(50.0)}}, actual5));
