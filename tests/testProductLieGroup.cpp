...
struct Rot3VectorAction : public GroupAction<Rot3VectorAction, Rot3, Vector3> {
  ...
  static Product Expmap(
      const Eigen::Ref<const typename traits<typename Product::FirstFactor>::
                           TangentVector>& w,
      const Eigen::Ref<const typename traits<typename Product::SecondFactor>::
                           TangentVector>& rho,
    ...
    return Product(pose.rotation(), pose.translation());
  }

  template <typename Product>
  static typename Product::TangentVector Logmap(
      const Product& p, typename Product::ChartJacobian H = {}) {
    Matrix6 Hpose;
    const Vector6 xi =
  ...
  template <typename Product>
  static typename Product::Jacobian AdjointMap(const Product& p) {
...
TEST(Lie, ProductLieGroupExplicitDirectAction) {
  ...
  const ProductExplicitDirect explicitState(state.first, state.second);
  const ProductExplicitDirect explicitOther(other.first, other.second);

  const Product defaultComposed = state.compose(other);
  const ProductExplicitDirect explicitComposed = explicitState.compose(explicitOther);
  EXPECT(assert_equal(defaultComposed.first, explicitComposed.first, kTol));
  EXPECT(assert_equal(defaultComposed.second, explicitComposed.second, kTol));

  const Product defaultBetween = state.between(other);
  const ProductExplicitDirect explicitBetween = explicitState.between(explicitOther);
  EXPECT(assert_equal(defaultBetween.first, explicitBetween.first, kTol));
  EXPECT(assert_equal(defaultBetween.second, explicitBetween.second, kTol));

  const Product defaultInverse = state.inverse();
  const ProductExplicitDirect explicitInverse = explicitState.inverse();
  EXPECT(assert_equal(defaultInverse.first, explicitInverse.first, kTol));
  EXPECT(assert_equal(defaultInverse.second, explicitInverse.second, kTol));

  Vector5 xi;
  xi << 0.1, -0.2, 0.05, 0.1, -0.15;
  const Product defaultExp = Product::Expmap(xi);
  const ProductExplicitDirect explicitExp = ProductExplicitDirect::Expmap(xi);
  EXPECT(assert_equal(defaultExp.first, explicitExp.first, kTol));
  EXPECT(assert_equal(defaultExp.second, explicitExp.second, kTol));
  EXPECT(assert_equal(Product::Logmap(defaultExp),
                      ProductExplicitDirect::Logmap(explicitExp), kTol));
  EXPECT(assert_equal(state.AdjointMap(), explicitState.AdjointMap(), kTol));
}

/* ************************************************************************* */
TEST(Lie, ProductLieGroupSemidirect) {
  ...
  const Vector3 t = Vector3(0.4, -0.5, 0.6);
...
/* ************************************************************************* */
TEST(testProductSemidirect, compose) {
...
/* ************************************************************************* */
TEST(testProductSemidirect, between) {
  const Semidirect state1(Rot3::RzRyRx(0.1, 0.2, -0.3),
                          Vector3(1.0, -0.5, 0.25));
  const Semidirect state2(Rot3::RzRyRx(-0.2, 0.1, 0.15),
                          Vector3(-0.75, 0.4, 1.2));
...
/* ************************************************************************* */
TEST(testProductSemidirect, inverse) {
...
/* ************************************************************************* */
TEST(testProductSemidirect, Expmap) {
  Vector6 xi;
  xi << 0.1, -0.2, 0.3, 0.4, -0.1, 0.2;
...
/* ************************************************************************* */
TEST(testProductSemidirect, Logmap) {
...
/* ************************************************************************* */
TEST(testProductSemidirect, AdjointMap) {
  const Semidirect state(Rot3::RzRyRx(0.1, -0.2, 0.3),
                         Vector3(0.4, -0.1, 0.2));
...
/* ************************************************************************* */
TEST(testProductSemidirect, retractAndLocalCoordinates) {
  const Semidirect state(Rot3::RzRyRx(0.1, -0.2, 0.3),
                         Vector3(0.4, -0.1, 0.2));
  Vector6 delta;
