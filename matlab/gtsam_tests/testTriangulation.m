...
upright = gtsam.Rot3.Ypr(-pi / 2, 0., -pi / 2);
...
triangulated_landmark = gtsam.triangulatePoint3( ...
    poses,sharedCal, measurements, rank_tol, optimize);
gtsam.CHECK('triangulated_landmark', abs(landmark - triangulated_landmark) < 1e-9);
...
triangulated_landmark = gtsam.triangulatePoint3( ...
    poses,sharedCal, measurements, rank_tol, optimize);
gtsam.CHECK('triangulated_landmark', abs(landmark - triangulated_landmark) < 1e-2);

%% two Poses with Bundler Calibration
bundlerCal = gtsam.Cal3Bundler(1500, 0, 0, 640, 480);
camera1 = gtsam.PinholeCameraCal3Bundler(pose1, bundlerCal);
camera2 = gtsam.PinholeCameraCal3Bundler(pose2, bundlerCal);
...
triangulated_landmark = gtsam.triangulatePoint3( ...
    poses,bundlerCal, measurements, rank_tol, optimize);
gtsam.CHECK('triangulated_landmark', abs(landmark - triangulated_landmark) < 1e-9);
