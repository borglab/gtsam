...
noiseModels.pose = gtsam.noiseModel.Diagonal.Sigmas( ...
    [0.001 0.001 0.001 0.1 0.1 0.1]', true);
%noiseModels.odometry = gtsam.noiseModel.Diagonal.Sigmas([0.001 0.001 0.001 0.1 0.1 0.1]');
noiseModels.odometry = gtsam.noiseModel.Diagonal.Sigmas( ...
    [0.05 0.05 0.05 0.2 0.2 0.2]', true);
noiseModels.point = gtsam.noiseModel.Isotropic.Sigma(3, 0.1, true);
noiseModels.measurement = gtsam.noiseModel.Isotropic.Sigma(2, 1.0, true);
    ...
    if i==1
        if options.hardConstraint % add hard constraint
            newFactors.add(gtsam.NonlinearEqualityPose3( ...
                ii,truth.cameras{1}.pose));
        else
            newFactors.add(gtsam.PriorFactorPose3( ...
                ii,truth.cameras{i}.pose, noiseModels.pose));
        end
    end
...
for i=1:2
    ...
    for k=1:length(data.Z{i})
        j = data.J{i}{k};
        jj = gtsam.symbol('l',data.J{i}{k});
        newFactors.add(gtsam.GenericProjectionFactorCal3_S2( ...
            data.Z{i}{k}, noiseModels.measurement, ii, jj, data.K));
        ...
        if options.pointPriors % add point priors
            newFactors.add(gtsam.PriorFactorPoint3( ...
                jj, truth.points{j}, noiseModels.point));
        end
    end
end
...
newFactors.add(gtsam.BetweenFactorPose3( ...
    gtsam.symbol('x',1), gtsam.symbol('x',2), ...
    data.odometry{1}, noiseModels.odometry));
