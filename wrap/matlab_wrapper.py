...
class MatlabWrapper(object):
    ...
    not_check_type = ['int', 'double', 'bool', 'char']

    """The static methods generated by serialize()
        (name, parameters, return type)
        return type of None means that it returns the class
    """
    static_methods = [
        ('StaticFunctionRet', ['double z'], None),
        ('StaticFunction', [], 'double')
    ]
    ...
    def _format_type_name(self, type_name):
        """ Format the given type name for wrap """
        formatted_type_name = ''

        if type_name.name in self.namespace_type:
    ...
    def _wrap_method_check_statement(self, args):
        ...
        for arg in self._wrap_args(args).split(','):
            ctype = arg.split()

            if len(ctype) == 0:
                continue

            if ctype[0] in self.not_check_type or \
                    (ctype[0] == 'unsigned' and
                        ctype[1] in self.not_check_type):
                continue

            if check_statement == '':
                check_statement = 'if length(varargin) == 1'
        ...
        for i, p in enumerate(param_list):
            ...
            if len(p.args_list) == 0:
                ...
                param_wrap += str(len(p.args_list)) \
                    + self._wrap_variable_arguments(p) + '\n'
    ...
    def wrap_class_methods(self, methods, serialize=[False]):
        ...
        Keyword arguments:
        methods -- list of the Methods to wrap
        ...
        for method in methods:
            ...
            if method.name == 'serialize':
                ...
                end_statement = '' if check_statement == '' else '  else'\
                    '\n'\
                ...
                method_text += '{method_args}) : returns {return_type}\n'\
                    ...
                    '  {check_statement}'\
                    '{spacing}{varargout}geometry_wrapper({num}, this, '\
    ...
    def wrap_static_methods(self, class_name, serialize):
        method_text = 'methods(Static = true)\n'

        if not serialize:
            return method_text

        for method_name, parameters, return_type in self.static_methods:
            return_type = class_name if return_type is None else return_type

            if len(parameters) == 0:
                param_text = ''
                isa = ''
            else:
                param_text = reduce(lambda x, y: x + ', ' + y, parameters)
                isa = ''

                for i, param in enumerate(parameters, 1):
                    isa += " && isa(varargin{{{num}}},'{ctype}'".format(
                        num=i, ctype=param.split()[0]
                    )

                isa += ')'

            method_text += '  function varargout = {name}(varargin)\n'\
                '    % {name_caps} usage: {name}({params}) : returns '\
                '{return_type}\n'\
                '    % Doxygen can be found at http://research.cc.gatech'\
                '.edu/borg/sites/edu.borg/html/index.html\n    % \n'\
                '    % Usage\n'\
                '    % {name_caps}({params})\n'\
                '    if length(varargin) == {param_num}{isa}\n'\
                '      varargout{{1}} = geometry_wrapper({num}, '\
                'varargin{{:}});\n'\
                '    else\n'\
                "      error('Arguments do not match any overload of "\
                "function Point3.{name}');\n"\
                '    end\n  end\n\n'.format(
                    name=method_name, name_caps=method_name.upper(),
                    param_num=len(parameters), params=param_text, isa=isa,
                    return_type=return_type, num=self.wrapper_count)
    ...
    def wrap_instantiated_class(self, instantiated_class):
        ...
        if len(instantiated_class.methods) != 0:
            ...
            content_text += '    ' + reduce(
                lambda x, y: x + '\n' + ('' if y == '' else '    ') + y,
                self.wrap_class_methods(methods, serialize=serialize)
                    .splitlines()
            ) + '\n'
        ...
        content_text += '  end\n\n  ' + reduce(
            self._insert_spaces,
            self.wrap_static_methods(instantiated_class.name,
