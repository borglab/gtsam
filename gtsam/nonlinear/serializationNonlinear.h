 ...
 *
 * @brief Global functions for performing serialization, designed for use with
 * matlab
 *
 * @date Jun 12, 2013
...
NonlinearFactorGraph::shared_ptr deserializeGraph(
...
NonlinearFactorGraph::shared_ptr deserializeGraphXML(
    const std::string& serialized_graph, const std::string& name = "graph");
...
std::string serializeValuesXML(const Values& values,
...
// Serialize
bool serializeGraphToFile(const NonlinearFactorGraph& graph,
...
bool serializeValuesToXMLFile(const Values& values, const std::string& fname,
                              const std::string& name = "values");

// Deserialize
NonlinearFactorGraph::shared_ptr deserializeGraphFromFile(
    const std::string& fname);
NonlinearFactorGraph::shared_ptr deserializeGraphFromXMLFile(
    const std::string& fname, const std::string& name = "graph");

Values::shared_ptr deserializeValuesFromFile(const std::string& fname);
Values::shared_ptr deserializeValuesFromXMLFile(
