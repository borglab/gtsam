...
struct CudaJacobianNormalOperator::Impl {
  ...
  CudaDeviceArray<double> intermediate;
  CudaDeviceArray<unsigned char> forwardWorkspace;
  CudaDeviceArray<unsigned char> transposeWorkspace;
  int rows = 0;
  int columns = 0;
  double lambda = 0.0;
...
CudaJacobianNormalOperator::CudaJacobianNormalOperator()
    : impl_(std::make_unique<Impl>()) {}
CudaJacobianNormalOperator::~CudaJacobianNormalOperator() = default;
CudaJacobianNormalOperator::CudaJacobianNormalOperator(
    CudaJacobianNormalOperator&&) noexcept = default;
CudaJacobianNormalOperator& CudaJacobianNormalOperator::operator=(
    CudaJacobianNormalOperator&&) noexcept = default;

void CudaJacobianNormalOperator::initialize(
  ...
  if (!handle || !jacobian || !jacobianTranspose || rows <= 0 || columns <= 0) {
    throw std::invalid_argument(
        "CudaJacobianNormalOperator requires valid descriptors and positive "
        "dimensions");
  }
...
void CudaJacobianNormalOperator::setDamping(
    double lambda, const CudaDeviceArray<double>& dampingDiagonal,
    cudaStream_t stream) {
  if (impl_->columns <= 0) {
    throw std::logic_error("CudaJacobianNormalOperator is not initialized");
  }
  if (stream != impl_->fixedStream || !std::isfinite(lambda) || lambda < 0.0 ||
      dampingDiagonal.size() != static_cast<size_t>(impl_->columns)) {
    throw std::invalid_argument(
        "CudaJacobianNormalOperator received invalid damping or stream");
  }
  impl_->lambda = lambda;
  impl_->dampingDiagonal = dampingDiagonal.data();
}

int CudaJacobianNormalOperator::dimension() const { return impl_->columns; }

void CudaJacobianNormalOperator::apply(const double* input, double* output,
                                       cudaStream_t stream) const {
  if (!input || !output || !impl_->dampingDiagonal ||
      stream != impl_->fixedStream) {
    throw std::invalid_argument(
        "CudaJacobianNormalOperator apply requires prepared storage and the "
        "fixed stream");
  }
  ...
  CheckCuda(cudaGetLastError(), "damping tail launch");
}

struct CudaJacobianNormalPreconditioner::Impl {
  int columns = 0;
...
CudaJacobianNormalPreconditioner::CudaJacobianNormalPreconditioner()
    : impl_(std::make_unique<Impl>()) {}
CudaJacobianNormalPreconditioner::~CudaJacobianNormalPreconditioner() = default;
CudaJacobianNormalPreconditioner::CudaJacobianNormalPreconditioner(
    CudaJacobianNormalPreconditioner&&) noexcept = default;
CudaJacobianNormalPreconditioner&
CudaJacobianNormalPreconditioner::operator=(
    CudaJacobianNormalPreconditioner&&) noexcept = default;

void CudaJacobianNormalPreconditioner::initialize(
    int columns, const CudaDeviceArray<int>& transposeRowPointers,
  ...
  if (columns <= 0 || transposeRowPointers.size() !=
    ...
    throw std::invalid_argument(
        "CudaJacobianNormalPreconditioner has invalid dimensions or blocks");
  }
  for (size_t i = 1; i < blockOffsets.size(); ++i) {
    const int width = blockOffsets[i] - blockOffsets[i - 1];
    if (width <= 0 || width > kMaxBlockWidth) {
      throw std::invalid_argument(
          "CudaJacobianNormalPreconditioner block widths must be in [1,16]");
    }
  }
  auto state = std::make_unique<Impl>();
  state->columns = columns;
  ...
  CheckCuda(cudaMemcpyAsync(hostRows.data(), transposeRowPointers.data(),
                            sizeof(int) * hostRows.size(),
                            cudaMemcpyDeviceToHost, stream),
            "row-pointer validation download");
  CheckCuda(cudaStreamSynchronize(stream), "row-pointer validation sync");
    ...
    for (int column = begin + 1; column < end; ++column) {
      if (hostRows[column + 1] - hostRows[column] != expected) {
        throw std::runtime_error(
            "CudaJacobianNormalPreconditioner requires equal transpose "
...
void CudaJacobianNormalPreconditioner::build(
    const CudaDeviceArray<double>& transposeValues, cudaStream_t stream) {
  if (impl_->columns <= 0 || stream != impl_->fixedStream) {
    throw std::logic_error(
        "CudaJacobianNormalPreconditioner is not initialized on this stream");
  }
  ...
  if (impl_->type == DevicePcgPreconditioner::BlockJacobi) {
    for (WidthClass& widthClass : impl_->widthClasses) {
      ...
      CheckCuda(cudaGetLastError(), "Gram block build launch");
    }
    ...
    CheckCuda(cudaGetLastError(), "Jacobi diagonal build launch");
  }
  impl_->built = true;
  impl_->prepared = false;
  if (impl_->collectProfile) {
    CheckCuda(cudaStreamSynchronize(stream), "profiled build sync");
    impl_->buildTime +=
...
void CudaJacobianNormalPreconditioner::prepare(
    double lambda, const CudaDeviceArray<double>& dampingDiagonal,
    cudaStream_t stream) {
  if (!impl_->built || stream != impl_->fixedStream ||
      ...
      dampingDiagonal.size() != static_cast<size_t>(impl_->columns)) {
    throw std::invalid_argument(
        "CudaJacobianNormalPreconditioner requires built data and valid "
        "damping");
  }
  const auto start = std::chrono::steady_clock::now();
  impl_->lambda = lambda;
  ...
  if (impl_->type == DevicePcgPreconditioner::BlockJacobi) {
    for (WidthClass& widthClass : impl_->widthClasses) {
      ...
      CheckCuda(cudaGetLastError(), "damped block inversion launch");
    }
  }
  impl_->prepared = true;
  if (impl_->collectProfile) {
    CheckCuda(cudaStreamSynchronize(stream), "profiled prepare sync");
    impl_->prepareTime +=
        std::chrono::duration<double>(std::chrono::steady_clock::now() - start)
            .count();
  }
...
int CudaJacobianNormalPreconditioner::dimension() const {
  return impl_->columns;
}

void CudaJacobianNormalPreconditioner::apply(const double* input,
                                             double* output,
                                             cudaStream_t stream) const {
  if (!impl_->prepared || !input || !output || stream != impl_->fixedStream) {
    throw std::invalid_argument(
        "CudaJacobianNormalPreconditioner apply requires prepared storage "
  ...
  switch (impl_->type) {
    case DevicePcgPreconditioner::BlockJacobi:
      for (const WidthClass& widthClass : impl_->widthClasses) {
        ...
        CheckCuda(cudaGetLastError(), "block preconditioner apply launch");
      }
      break;
    case DevicePcgPreconditioner::Jacobi:
      ...
      CheckCuda(cudaGetLastError(), "Jacobi preconditioner apply launch");
      break;
    case DevicePcgPreconditioner::None:
      CheckCuda(cudaMemcpyAsync(output, input,
                                sizeof(double) *
...
double CudaJacobianNormalPreconditioner::buildSeconds() const {
  return impl_->buildTime;
}

double CudaJacobianNormalPreconditioner::prepareSeconds() const {
