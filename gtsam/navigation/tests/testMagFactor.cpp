...
TEST( MagFactor, Factors ) {
  ...
  EXPECT( assert_equal(Z_3x1,f.evaluateError(theta,H1),GTSAM_UNITTEST_TOLERANCE_LOOSE));
...
// MagFactor1
  ...
  EXPECT( assert_equal(Z_3x1,f1.evaluateError(nRb,H1),GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT( assert_equal(numericalDerivative11<Vector,Rot3> //
      (boost::bind(&MagFactor1::evaluateError, &f1, _1, none), nRb), H1, 1e-7));

// MagFactor2
  MagFactor2 f2(1, 2, measured, nRb, model);
  EXPECT( assert_equal(Z_3x1,f2.evaluateError(scaled,bias,H1,H2),GTSAM_UNITTEST_TOLERANCE_LOOSE));
...
// MagFactor2
  ...
  EXPECT(assert_equal(Z_3x1,f3.evaluateError(s,dir,bias,H1,H2,H3),GTSAM_UNITTEST_TOLERANCE_LOOSE));
