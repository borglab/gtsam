...
//[MH-A]: OLD GaussianConditional inherits JacobianFactor and therefore cannot be used in MH...
  //class GTSAM_EXPORT GaussianConditional : public JacobianFactor, public Conditional<JacobianFactor, GaussianConditional> {
//[MH-A]: NEW GaussianConditional inherits MHJacobianFactor and therefore can be used in MH... 
  class GTSAM_EXPORT GaussianConditional : public MHJacobianFactor, public Conditional<MHJacobianFactor, GaussianConditional> {
  ...
  public:
    ...
    typedef MHJacobianFactor BaseFactor; ///< Typedef to our factor base class
    ...
    //  is used, and that the matrix data is copied into a newly-allocated matrix in the constructed factor.
    template<typename KEYS>
    GaussianConditional(
      const KEYS& keys, size_t nrFrontals, const VerticalBlockMatrix& augmentedMatrix,
      const SharedDiagonal& sigmas = SharedDiagonal()); //mhsiao: used in HessianFactor::eliminateCholesky()

    //[MH-A]: constructor for MHJacobianFactor inputs
    template<typename KEYS>
    GaussianConditional(const KEYS& keys, size_t nrFrontals, const std::list<VerticalBlockMatrix>& augmentedMatrix_list, HypoLayer* resulting_layer, const SharedDiagonal& sigmas = SharedDiagonal()); //mhsiao: used in MHHessianFactor::mhEliminateCholesky()
    ...
    //[MH-A]:
    constABlock mhGet_single_R(JacobListCstIter& jit) const { return ((*jit).matrixObject()).range(0, nrFrontals()); }

    // Get a view of the parent blocks.
    constABlock get_S() const { return Ab_.range(nrFrontals(), size()); }
    //[MH-A]:
    constABlock mhGet_single_S(JacobListCstIter& jit) const { return ((*jit).matrixObject()).range(nrFrontals(), size()); }
    ...
    //[MH-A]:
    const constBVector mhGet_single_d(JacobListCstIter& jit) const { return (*jit).getb(); }

    //[MH-C]:
    double mhGet_single_err2(JacobListCstIter& jit) const {
      const size_t err_idx = nrFrontals(); // get e^2 at the bottom-left 
      //return ((*jit).matrixObject()).matrix()(err_idx, err_idx);
      return pow( ((*jit).matrixObject()).matrix()(err_idx, err_idx) , 2 );
    }
    ...
    //[MH-A]: solve MH-case
    VectorValues mhSolve(Values& theta, const VectorValues& parents, const double& splitThreshold) const;
    ...
    // Scale the values in \c gy according to the sigmas for the frontal variables in this conditional.
