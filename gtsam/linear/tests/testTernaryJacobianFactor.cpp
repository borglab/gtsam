...
/**
 * @file testTernaryJacobianFactor.cpp
 * @brief Tests for fixed-size TernaryJacobianFactor operations.
 */
...
using Factor = TernaryJacobianFactor<2, 3, 2, 1>;
...
Factor fixedFactor(const SharedDiagonal& model) {
  return Factor(kKey1, kA1, kKey2, kA2, kKey3, kA3, kB, model);
}
...
TEST(TernaryJacobianFactor, DeltaErrorMatchesGeneric) {
...
TEST(TernaryJacobianFactor, HessianDiagonalMatchesGeneric) {
  for (const SharedDiagonal& model : noiseModels()) {
    const Factor actual = fixedFactor(model);
    const JacobianFactor expected = genericFactor(model);
    const GaussianFactor& actualBase = actual;
...
TEST(TernaryJacobianFactor, HessianDiagonalAccumulates) {
  for (const SharedDiagonal& model : noiseModels()) {
    const Factor actual = fixedFactor(model);
    const JacobianFactor expected = genericFactor(model);
    const GaussianFactor& actualBase = actual;
...
TEST(TernaryJacobianFactor, RangedUpdateMatchesWholeAndGeneric) {
...
TEST(TernaryJacobianFactor, ConstrainedRangedUpdateThrows) {
