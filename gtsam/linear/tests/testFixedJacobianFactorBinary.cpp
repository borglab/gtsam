...
/**
 * @file testFixedJacobianFactorBinary.cpp
 * @brief Tests for binary-arity FixedJacobianFactor operations.
 */
...
/* ************************************************************************* */
namespace fixed_jacobian_factor_binary {

using Factor = FixedJacobianFactor<2, 3, 2>;
...
const KeyVector kKeys{kKey1, kKey2};
const std::vector<Matrix> kJacobians{kA1, kA2};
...
TEST(FixedJacobianFactorBinary, DeltaErrorMatchesGeneric) {
...
TEST(FixedJacobianFactorBinary, HessianDiagonalMatchesGeneric) {
  for (const SharedDiagonal& model : noiseModels()) {
    const Factor actual = fixedFactor(model);
    const JacobianFactor expected(kKey1, kA1, kKey2, kA2, kB, model);
    const GaussianFactor& actualBase = actual;
...
TEST(FixedJacobianFactorBinary, HessianDiagonalAccumulates) {
  for (const SharedDiagonal& model : noiseModels()) {
    const Factor actual = fixedFactor(model);
    const JacobianFactor expected(kKey1, kA1, kKey2, kA2, kB, model);
    const GaussianFactor& actualBase = actual;
...
TEST(FixedJacobianFactorBinary, RangedUpdateMatchesWholeAndGeneric) {
  for (const SharedDiagonal& model : noiseModels()) {
...
TEST(FixedJacobianFactorBinary, ScalarBlocksMatchGeneric) {
  using ScalarFactor = FixedJacobianFactor<3, 1, 1>;
  const Eigen::Matrix<double, 3, 1> A1{1.0, -2.0, 0.5};
  const Eigen::Matrix<double, 3, 1> A2{-0.25, 3.0, 1.5};
  const Vector3 b{0.75, -1.0, 2.0};
  const ScalarFactor actual(KeyVector{kKey1, kKey2},
                            std::vector<Matrix>{A1, A2}, b);
...
TEST(FixedJacobianFactorBinary, ConstrainedRangedUpdateThrows) {
  const SharedDiagonal model = noiseModel::Constrained::All(2);
  const Factor factor = fixedFactor(model);
...
TEST(FixedJacobianFactorBinary, UnlistedDimensionsUseInlineFallback) {
  using UnlistedFactor = FixedJacobianFactor<4, 2, 4>;
  ...
  const UnlistedFactor actual(KeyVector{kKey1, kKey2},
                              std::vector<Matrix>{A1, A2}, b);
  const JacobianFactor expected(kKey1, A1, kKey2, A2, b);
  const GaussianFactor& actualBase = actual;
