...
TEST(Unit3, point3) {
  ...
  for(Point3 p: ps) {
    ...
    EXPECT(assert_equal(p, s.point3(actualH), GTSAM_UNITTEST_TOLERANCE_LOOSE));
...
TEST(Unit3, dot) {
  ...
  EXPECT(assert_equal(1.0, p.dot(p), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal(0.877583, p.dot(q), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal(0.696707, p.dot(r), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal(0.955336, p.dot(t), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  ...
  {
    p.dot(q, H1, H2);
    EXPECT(assert_equal(numericalDerivative21<double,Unit3>(f, p, q), H1, GTSAM_UNITTEST_TOLERANCE_LOOSE));
    EXPECT(assert_equal(numericalDerivative22<double,Unit3>(f, p, q), H2, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
    p.dot(r, H1, H2);
    EXPECT(assert_equal(numericalDerivative21<double,Unit3>(f, p, r), H1, GTSAM_UNITTEST_TOLERANCE_LOOSE));
    EXPECT(assert_equal(numericalDerivative22<double,Unit3>(f, p, r), H2, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
    p.dot(t, H1, H2);
    EXPECT(assert_equal(numericalDerivative21<double,Unit3>(f, p, t), H1, GTSAM_UNITTEST_TOLERANCE_LOOSE));
    EXPECT(assert_equal(numericalDerivative22<double,Unit3>(f, p, t), H2, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
}
...
TEST(Unit3, error) {
  ...
  EXPECT(assert_equal((Vector)(Vector2(0, 0)), p.error(p), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal((Vector)(Vector2(0.479426, 0)), p.error(q), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal((Vector)(Vector2(0.717356, 0)), p.error(r), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    ...
    p.error(q, actual);
    EXPECT(assert_equal(expected.transpose(), actual, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
    expected = numericalDerivative11<Vector2,Unit3>(
...
TEST(Unit3, error2) {
  ...
  EXPECT(assert_equal((Vector)(Vector2(0.0, 0.0)), p.errorVector(p), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal((Vector)(Vector2(0.198337495, -0.0991687475)), p.errorVector(q), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  EXPECT(assert_equal((Vector)(Vector2(0.717356, 0)), p.errorVector(r), GTSAM_UNITTEST_TOLERANCE_LOOSE));
...
TEST(Unit3, distance) {
  Unit3 p(1, 0, 0), q = p.retract(Vector2(0.5, 0)), //
  r = p.retract(Vector2(0.8, 0));
  EXPECT_DOUBLES_EQUAL(0, p.distance(p), GTSAM_UNITTEST_TOLERANCE_LOOSE);
  EXPECT_DOUBLES_EQUAL(0.47942553860420301, p.distance(q), GTSAM_UNITTEST_TOLERANCE_LOOSE);
  EXPECT_DOUBLES_EQUAL(0.71735609089952279, p.distance(r), GTSAM_UNITTEST_TOLERANCE_LOOSE);
...
TEST(Unit3, localCoordinates0) {
  ...
  EXPECT(assert_equal(Z_2x1, actual, GTSAM_UNITTEST_TOLERANCE_LOOSE));
}
...
TEST(Unit3, localCoordinates) {
  {
    ...
    EXPECT(assert_equal((Vector) Z_2x1, actual, GTSAM_UNITTEST_TOLERANCE_LOOSE));
    EXPECT(assert_equal(q, p.retract(expected), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
  ...
  }
  {
    ...
    Vector2 actual = p.localCoordinates(q);
    EXPECT(assert_equal(q, p.retract(actual), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
...
TEST(Unit3, retract) {
  {
    ...
    EXPECT(assert_equal(v, p.localCoordinates(actual), GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
    Unit3 p;
    Vector2 v(0, 0);
...
TEST (Unit3, jacobian_retract) {
      ...
      boost::bind(&Unit3::retract, p, _1, boost::none);
  {
      ...
      EXPECT(assert_equal(H_expected_numerical, H, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
  {
      Vector2 v (0, 0);
      p.retract(v, H);
...
TEST(Unit3, localCoordinates_retract) {
  ...
  for (size_t i = 0; i < numIterations; i++) {
    ...
    EXPECT(assert_equal(s2, actual_s2, GTSAM_UNITTEST_TOLERANCE_LOOSE));
  }
}
...
TEST (Unit3, FromPoint3) {
  ...
  EXPECT(assert_equal(expected, Unit3::FromPoint3(point, actualH), GTSAM_UNITTEST_TOLERANCE_LOOSE));
