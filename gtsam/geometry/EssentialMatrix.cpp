...
/* ************************************************************************* */
Vector3 EssentialMatrix::pointToEpipolarLine(const Vector3& p, OptionalJacobian<3, 5> DE) const {
  if(DE) {
    // See math.lyx
    Matrix3 HR = E_ * skewSymmetric(-p);
    Matrix32 HD = skewSymmetric(-rotation().matrix() * p) * direction().basis();
    *DE << HR, HD;
  }
  return E_ * p;
}
