...
double SIGKernel::updateMu(const double& mu, const double& residual,
  ...
  if (params_.mu_update_strat == MuUpdateStrategy::MCGANN_2023) {
    ...
    // Simple mu update of [mu_init, 0.5, 1.0] where 0.5 results in
    // approximately the huber kernel
    ...
    if (!params_.kang_residual_threshold) {
      throw std::runtime_error(
          "mu_update_strat == KANG_CODE_2023 but no kang_residual_threshold "
          "provided in params");
    }
    if (update_count == 0 && residual > params_.shape_param / std::sqrt(3) &&
      ...
      // return 1.0 If r < c/sqrt(3) then the result will be > 1.0 so skip
      // search and return 1.0 If r > kang_residual_threshold after the convex
      // iter then this measurement is most def an outlier so go to 1.0 Note:
      // This final condition is present in the code, but not in the paper by
      // Kang et al.
      return 1.0;
    }
    ...
    // Conditional Arithmetic Series: mu[t+1] = mu[t] + 0.5 if (t==0) else mu[t]
    // + 0.25
    if (update_count == 0) return std::min(1.0, mu + 0.5);
    return std::min(1.0, mu + 0.25);
  } else if (params_.mu_update_strat == MuUpdateStrategy::NONCONVEX) {
    // Conditional Arithmetic Series: mu[t+1] = mu[t] + 0.7 if (t==0) else mu[t]
    // + 0.2
    ...
    // 0.4 if (t==1), else mu[t] + 0.05 My attempt to build the most stable
    // method possible given lessons learned from multi-robot conditions
    if (update_count == 0) return std::min(1.0, mu + 0.5);
    if (update_count == 1) return std::min(1.0, mu + 0.4);
    return std::min(1.0, mu + 0.05);
  } else {
    throw std::runtime_error(
        "updateMu: requested MuUpdateStrategy is not implemented.");
  }
}
...
double SIGKernel::incrementMuInit(const double& mu) const {
  if (params_.mu_init_inc_strat == MuInitIncrementStrategy::MCGANN_2023) {
    ...
    throw std::runtime_error(
        "incrementMuInit: requested MuInitIncrementStrategy is not "
        "implemented.");
  }
}
...
double SIGKernel::incrementMuInitInv(const double& mu) const {
  if (params_.mu_init_inc_strat == MuInitIncrementStrategy::MCGANN_2023) {
    ...
    throw std::runtime_error(
        "incrementMuInitInv: requested MuInitIncrementStrategy is not "
        "implemented.");
  }
}
