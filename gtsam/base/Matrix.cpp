...
/* ************************************************************************* */
bool assert_equal(const std::list<Matrix>& As, const std::list<Matrix>& Bs,
                  double tol) {
  ...
  for (; itA != As.end(); itA++, itB++)
    if (!assert_equal(*itB, *itA, tol)) return false;
...
Vector operator^(const Matrix& A, const Vector& v) {
  if (A.rows() != v.size())
    throw std::invalid_argument(
        boost::str(boost::format("Matrix operator^ : A.m(%d)!=v.size(%d)") %
                   A.rows() % v.size()));
...
void print(const Matrix& A, const string& s) { print(A, s, cout); }
...
istream& operator>>(istream& inputStream, Matrix& destinationMatrix) {
  ...
  for (const vector<double>& rowVec : coeffs) {
    destinationMatrix.row(row) =
        Eigen::Map<const Eigen::RowVectorXd>(&rowVec[0], width);
    ++row;
  }
...
pair<Matrix, Matrix> qr(const Matrix& A) {
  ...
  for (size_t j = 0; j < kprime; j++) {
    ...
    Vector xjm(mm);
    for (size_t k = 0; k < mm; k++) xjm(k) = R(j + k, j);
    ...
    for (size_t k = 0; k < m; k++) v(k) = k < j ? 0.0 : vjm(k - j);
...
/* ************************************************************************* */
list<boost::tuple<Vector, double, double> > weighted_eliminate(
    Matrix& A, Vector& b, const Vector& sigmas) {
  ...
  for (size_t j = 0; j < n; ++j) {
    ...
    for (size_t j2 = j + 1; j2 < n; ++j2) r(j2) = pseudo.dot(A.col(j2));
...
void householder_(Matrix& A, size_t k, bool copy_vectors) {
  ...
  for (size_t j = 0; j < kprime; j++) {
    ...
    // do outer product update A(j:m,:) = (I-beta vv')*A = A - v*(beta*A'*v)' =
    // A - v*w'
...
Vector backSubstituteUpper(const Vector& b, const Matrix& U, bool unit) {
  ...
  assert(U.rows() == U.cols());
  if (unit)
    return U.triangularView<Eigen::UnitUpper>()
        .transpose()
        .solve<Eigen::OnTheLeft>(b);
  else
    return U.triangularView<Eigen::Upper>().transpose().solve<Eigen::OnTheLeft>(
        b);
}
...
void vector_scale_inplace(const Vector& v, Matrix& A, bool inf_mask) {
  ...
  if (inf_mask) {
    for (DenseIndex i = 0; i < m; ++i) {
      const double& vi = v(i);
      if (std::isfinite(vi)) A.row(i) *= vi;
    }
  } else {
    for (DenseIndex i = 0; i < m; ++i) A.row(i) *= v(i);
  }
}
...
Matrix vector_scale(const Matrix& A, const Vector& v, bool inf_mask) {
  ...
  if (inf_mask) {
    for (size_t j = 0; j < n; ++j) {
      const double& vj = v(j);
      if (std::isfinite(vj)) M.col(j) *= vj;
    }
  } else {
    for (size_t j = 0; j < n; ++j) M.col(j) *= v(j);
  }
  return M;
}
...
Matrix expm(const Matrix& A, size_t K) {
  Matrix E = Matrix::Identity(A.rows(), A.rows()),
         A_k = Matrix::Identity(A.rows(), A.rows());
...
std::string formatMatrixIndented(const std::string& label, const Matrix& matrix,
  ...
  const bool transposeMatrix =
      makeVectorHorizontal && matrix.cols() == 1 && matrix.rows() > 1;
  const DenseIndex effectiveRows =
      transposeMatrix ? matrix.cols() : matrix.rows();
  ...
  if (matrix.rows() > 0 && matrix.cols() > 0) {
    ...
    boost::tokenizer<boost::char_separator<char> > tok(
        matrixStr, boost::char_separator<char>("\n"));
...
void inplace_QR(Matrix& A) {
  ...
  Eigen::internal::householder_qr_inplace_blocked<Matrix, HCoeffsType>(
      A, hCoeffs, 48, temp.data());
#else
  Eigen::internal::householder_qr_inplace_blocked<Matrix, HCoeffsType>::run(
      A, hCoeffs, 48, temp.data());
#endif
