 ...
 * should exist and return a collection of (shared) pointers to \c FOREST::Node.
 *  @param visitorPre \c visitorPre(node, parentData) will be called at every
 * node, before visiting its children, and will be passed, by reference, the \c
 * DATA object returned by the visit to its parent.  Likewise, \c visitorPre
 * should return the \c DATA object to pass to the children.  The returned \c
 * DATA object will be copy-constructed only upon returning to store internally,
 * thus may be modified by visiting the children. Regarding efficiency, this
 * copy-on-return is usually optimized out by the compiler.
 *  @param visitorPost \c visitorPost(node, data) will be called at every node,
 * after visiting its children, and will be passed, by reference, the \c DATA
 * object returned by the call to \c visitorPre (the \c DATA object may be
 * modified by visiting the children).
 *  @param rootData The data to pass by reference to \c visitorPre when it is
 * called on each root node. */
...
void DepthFirstForest(FOREST& forest, DATA& rootData, VISITOR_PRE& visitorPre,
  ...
  Stack stack;
  FastList<DATA> dataList;  // List to store node data as it is returned from
                            // the pre-order visitor

  // Add roots to stack (insert such that they are visited and processed in
  // order
  {
  ...
  while (!stack.empty()) {
    ...
    if (node.expanded) {
      // If already expanded, then the data stored in the node is no longer
      // needed, so visit then delete it.
      (void)visitorPost(node.treeNode, *node.dataPointer);
      dataList.erase(node.dataPointer);
      stack.pop_front();
    } else {
      // If not already visited, visit the node and add its children (use
      // reverse iterators so children are processed in the order they appear)
      node.dataPointer = dataList.insert(
          dataList.end(), visitorPre(node.treeNode, node.parentData));
      typename Stack::iterator insertLocation = stack.begin();
...
/** Traverse a forest depth-first, with a pre-order visit but no post-order
 * visit.
 *  @param forest The forest of trees to traverse.  The method \c forest.roots()
 * should exist and return a collection of (shared) pointers to \c FOREST::Node.
 *  @param visitorPre \c visitorPre(node, parentData) will be called at every
 ...
 * setting up child pointers for a clone of the original tree.
 *  @param forest The forest of trees to clone.  The method \c forest.roots()
 * should exist and return a collection of shared pointers to \c FOREST::Node.
...
struct PrintForestVisitorPre {
  ...
  template <typename NODE>
  std::string operator()(const boost::shared_ptr<NODE>& node,
...
/** Print a tree, prefixing each line with \c str, and formatting keys using \c
 * keyFormatter. To print each node, this function calls the \c print function
 * of the tree nodes. */
template <class FOREST>
