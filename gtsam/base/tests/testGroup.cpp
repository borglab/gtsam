...
class Symmetric : private Eigen::PermutationMatrix<N> {
 ...
 public:
  ...
  Symmetric() { Eigen::PermutationMatrix<N>::setIdentity(); }
  ...
  friend std::ostream& operator<<(std::ostream& os, const Symmetric& m) {
    for (size_t i = 0; i < N; i++) os << m.indices()[i] << " ";
    return os;
  }
...
/// Define permutation group traits to be a model of the Multiplicative Group
/// concept
template <int N>
struct traits<Symmetric<N> >
    : internal::MultiplicativeGroupTraits<Symmetric<N> >,
