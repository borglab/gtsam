...
size_t DSFBase::find(size_t key) const {
  ...
  if (parent != key) parent = find(parent);  // recursive call
...
/* ************************************************************************* */
DSFVector::DSFVector(const size_t numNodes) : DSFBase(numNodes) {
  keys_.reserve(numNodes);
  for (size_t index = 0; index < numNodes; index++) keys_.push_back(index);
}
...
std::set<size_t> DSFVector::set(const size_t& label) const {
  std::set<size_t> set;
  for (size_t key : keys_)
    if (find(key) == label) set.insert(key);
  return set;
}
...
std::map<size_t, std::set<size_t> > DSFVector::sets() const {
  ...
  for (size_t key : keys_) sets[find(key)].insert(key);
  return sets;
}
...
std::map<size_t, std::vector<size_t> > DSFVector::arrays() const {
  ...
  for (size_t key : keys_) arrays[find(key)].push_back(key);
  return arrays;
}
