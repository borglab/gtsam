...
/**
 * @file    statistics.h
 * @brief   Tools for gathering statistics about a forest to aid tuning parallel
 * traversal
...
namespace internal {
template <class NODE>
ForestStatistics* statisticsVisitor(const boost::shared_ptr<NODE>& node,
  ...
  if (node->children.size() > 1) {
    ...
    (*stats->problemSizeOfSecondLargestChildHistogram
          [secondLargestProblemSize])++;
  }
  return stats;
}
}  // namespace internal
...
ForestStatistics GatherStatistics(const FOREST& forest) {
  ...
  DepthFirstForest(forest, statsPtr,
                   internal::statisticsVisitor<typename FOREST::Node>);
  return stats;
}
