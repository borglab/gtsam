...
/* ************************************************************************* */
template <typename NODE, typename DATA, typename VISITOR_PRE,
...
class PreOrderTask : public tbb::task {
 public:
  ...
  PreOrderTask(const boost::shared_ptr<NODE>& treeNode,
               const boost::shared_ptr<DATA>& myData, VISITOR_PRE& visitorPre,
  ...
  tbb::task* execute() override {
    if (isPostOrderPhase) {
      // Run the post-order visitor since this task was recycled to run the
      // post-order visitor
      ...
      if (makeNewTasks) {
        if (!treeNode->children.empty()) {
          ...
          for (const boost::shared_ptr<NODE>& child : treeNode->children) {
            // Process child in a subtask.  Important:  Run visitorPre before
            // calling allocate_child so that if visitorPre throws an exception,
            // we will not have allocated an extra child, this causes a TBB
            // error.
          ...
          // If we have child tasks, start subtasks and wait for them to
          // complete
...
/* ************************************************************************* */
template <typename ROOTS, typename NODE, typename DATA, typename VISITOR_PRE,
          typename VISITOR_POST>
class RootTask : public tbb::task {
 public:
  ...
  VISITOR_POST& visitorPost;
  int problemSizeThreshold;
  RootTask(const ROOTS& roots, DATA& myData, VISITOR_PRE& visitorPre,
           VISITOR_POST& visitorPost, int problemSizeThreshold)
      : roots(roots),
        myData(myData),
        visitorPre(visitorPre),
  ...
  tbb::task* execute() override {
    ...
    for (const boost::shared_ptr<NODE>& root : roots) {
      ...
      tasks.push_back(*new (allocate_child()) PreOrderTask(
...
template <typename NODE, typename ROOTS, typename DATA, typename VISITOR_PRE,
          typename VISITOR_POST>
RootTask<ROOTS, NODE, DATA, VISITOR_PRE, VISITOR_POST>& CreateRootTask(
    const ROOTS& roots, DATA& rootData, VISITOR_PRE& visitorPre,
    VISITOR_POST& visitorPost, int problemSizeThreshold) {
  typedef RootTask<ROOTS, NODE, DATA, VISITOR_PRE, VISITOR_POST> RootTask;
  return *new (tbb::task::allocate_root())
