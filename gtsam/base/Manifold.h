...
/**
 * A manifold defines a space in which there is a notion of a linear tangent
 * space that can be centered around a given point on the manifold.  These
 * nonlinear spaces may have such properties as wrapping around (as is the case
 * with rotations), which might make linear operations on parameters not return
 * a viable element of the manifold.
 *
 * We perform optimization by computing a linear delta in the tangent space of
 * the current estimate, and then apply this change using a retraction
 * operation, which maps the change in tangent space back to the manifold
 * itself.
 *
 * There may be multiple possible retractions for a given manifold, which can be
 * chosen between depending on the computational complexity.  The important
 * criteria for the creation for the retract and localCoordinates functions is
 * that they be inverse operations.
 *
 */
 ...
 public:
  ...
  BOOST_CONCEPT_USAGE(IsManifold) {
    BOOST_STATIC_ASSERT_MSG(
        ...
        "This type's structure_category trait does not assert it as a manifold "
        "(or derived)");
...
/// Helper class to construct the product manifold of two other manifolds, M1
/// and M2 Deprecated because of limited usefulness, maximum obfuscation
...
class ProductManifold : public std::pair<M1, M2> {
 ...
 public:
  ...
  TangentVector localCoordinates(const ProductManifold& other) const {
    typename traits<M1>::TangentVector v1 =
        traits<M1>::Local(this->first, other.first);
    typename traits<M2>::TangentVector v2 =
        traits<M2>::Local(this->second, other.second);
    TangentVector v;
    v << v1, v2;
    return v;
  }
...
// Define any direct product group to be a model of the multiplicative Group
// concept
template <typename M1, typename M2>
struct traits<ProductManifold<M1, M2> >
    : internal::Manifold<ProductManifold<M1, M2> > {};
#endif
...
#define GTSAM_CONCEPT_MANIFOLD_TYPE(T) \
  typedef gtsam::IsManifold<T> _gtsam_IsManifold_##T;
