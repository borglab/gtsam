...
/**
 * OptionalJacobian is an Eigen::Ref like class that can take be constructed
 * using either a fixed size or dynamic Eigen matrix. In the latter case, the
 * dynamic matrix will be resized. Finally, there is a constructor that takes
 * boost::none, the default constructor acts like boost::none, and
 * boost::optional<Eigen::MatrixXd&> is also supported for backwards
 * compatibility. Below this class, a dynamic version is also implemented.
 */
template <int Rows, int Cols>
class OptionalJacobian {
 public:
  /// Jacobian size type
  /// TODO(frank): how to enforce RowMajor? Or better, make it work with any
  /// storage order?
 ...
 private:
  ...
  void usurp(double* data) { new (&map_) Eigen::Map<Jacobian>(data); }
 ...
 public:
  ...
  OptionalJacobian() : map_(nullptr) {}

  /// Constructor that will usurp data of a fixed-size matrix
  OptionalJacobian(Jacobian& fixed) : map_(nullptr) { usurp(fixed.data()); }

  /// Constructor that will usurp data of a fixed-size matrix, pointer version
  OptionalJacobian(Jacobian* fixedPtr) : map_(nullptr) {
    if (fixedPtr) usurp(fixedPtr->data());
  }

  /// Constructor that will resize a dynamic matrix (unless already correct)
  OptionalJacobian(Eigen::MatrixXd& dynamic) : map_(nullptr) {
  ...
  OptionalJacobian(boost::none_t /*none*/) : map_(nullptr) {}
  ...
  /// TODO(frank): unfortunately using a Map makes usurping non-contiguous
  /// memory impossible
  //  template <typename Derived, bool InnerPanel>
  //  OptionalJacobian(Eigen::Block<Derived,Rows,Cols,InnerPanel> block) :
  //  map_(nullptr) { ?? }
  ...
  /// Access Rows*N sub-block if we are allocated, otherwise return an empty
  /// OptionalJacobian The use case is functions with arguments that are
  /// dissected, e.g. Pose3 into Rot3, Point3
  /// TODO(frank): ideally, we'd like full block functionality, but see note
  /// above.
  template <int N>
  ...
  /// Access M*Cols sub-block if we are allocated, otherwise return empty
  /// OptionalJacobian The use case is functions that create their return value
  /// piecemeal by calling other functions
  /// TODO(frank): Unfortunately we assume column-major storage order and hence
  /// this can't work template <int M> OptionalJacobian<M, Cols> rows(int
  /// startRow) { ?? }
};
...
class OptionalJacobian<Eigen::Dynamic, Eigen::Dynamic> {
 ...
 public:
  /// Default constructor acts like boost::none
  OptionalJacobian() : pointer_(nullptr) {}
  ...
  /// Constructor with boost::none just makes empty
  OptionalJacobian(boost::none_t /*none*/) : pointer_(nullptr) {}
...
struct MakeJacobian {
  typedef Eigen::Matrix<double, traits<T>::dimension, traits<A>::dimension>
      type;
};
