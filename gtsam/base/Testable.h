 ...
 * @addtogroup base
 * @tparam T is the objectype this constrains to be testable - assumes print()
 * and equals()
 */
template <class T>
class IsTestable {
...
bool assert_equal(const V& expected, const V& actual, double tol = 1e-9) {
  if (traits<V>::Equals(actual, expected, tol)) return true;
  printf("Not equal:\n");
 ...
 */
template <class V>
struct equals_star : public std::function<bool(const boost::shared_ptr<V>&,
                                               const boost::shared_ptr<V>&)> {
  double tol_;
  equals_star(double tol = 1e-9) : tol_(tol) {}
  bool operator()(const boost::shared_ptr<V>& expected,
...
struct Testable {
  ...
  static void Print(const T& m, const std::string& str = "") { m.print(str); }
...
#define GTSAM_CONCEPT_TESTABLE_TYPE(T) \
  typedef gtsam::IsTestable<T> _gtsam_Testable_##T;
