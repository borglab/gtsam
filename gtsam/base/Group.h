 ...
 public:
  ...
  BOOST_CONCEPT_USAGE(IsGroup) {
    BOOST_STATIC_ASSERT_MSG(
        ...
        "This type's structure_category trait does not assert it as a group "
        "(or derived)");
    e = traits<G>::Identity();
...
check_group_invariants(const G& a, const G& b, double tol = 1e-9) {
  ...
  return traits<G>::Equals(traits<G>::Compose(a, traits<G>::Inverse(a)), e,
                           tol) &&
         traits<G>::Equals(traits<G>::Between(a, b),
                           traits<G>::Compose(traits<G>::Inverse(a), b), tol) &&
         traits<G>::Equals(traits<G>::Compose(a, traits<G>::Between(a, b)), b,
                           tol);
}

namespace internal {

/// A helper class that implements the traits interface for multiplicative
/// groups. Assumes existence of identity, operator* and inverse method
...
struct MultiplicativeGroupTraits {
  ...
  static Class Between(const Class& g, const Class& h) {
    return g.inverse() * h;
  }
...
template <class Class>
struct MultiplicativeGroup : MultiplicativeGroupTraits<Class>,
                             Testable<Class> {};
...
class DirectProduct : public std::pair<G, H> {
 ...
 public:
  ...
  DirectProduct()
      : std::pair<G, H>(traits<G>::Identity(), traits<H>::Identity()) {}
...
class DirectSum : public std::pair<G, H> {
 ...
 public:
  ...
  DirectSum operator-() const { return DirectSum(-g(), -h()); }
};
...
#define GTSAM_CONCEPT_GROUP_TYPE(T) \
  typedef gtsam::IsGroup<T> _gtsam_IsGroup_##T;
