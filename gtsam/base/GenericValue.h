...
class GenericValue : public Value {
 ...
 public:
  ...
  Value* clone_() const override {
    GenericValue* ptr =
        new GenericValue(*this);  // calls copy constructor to fill in
    return ptr;
  }
   ...
   * using clone_().
   */
  void deallocate_() const override { delete this; }

  /**
   * Clone this value (normal clone on the heap, delete with 'delete' operator)
   */
  boost::shared_ptr<Value> clone() const override {
    return boost::allocate_shared<GenericValue>(
        Eigen::aligned_allocator<GenericValue>(), *this);
  }
  ...
  size_t dim() const override { return traits<T>::GetDimension(value_); }
 ...
 private:
  ...
  void serialize(ARCHIVE& ar, const unsigned int /*version*/) {
    ar& boost::serialization::make_nvp(
        "GenericValue", boost::serialization::base_object<Value>(*this));
