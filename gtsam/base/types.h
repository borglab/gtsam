...
struct ValueWithDefault {
  ...
  /** Default constructor, initialize to default value supplied in template
   * argument */
...
#pragma clang diagnostic ignored \
    "-Wunused-private-field"  // Clang complains that previousOpenMPThreads is
                              // unused in the #else case below
#endif

/// An object whose scope defines a block where TBB and OpenMP parallelism are
/// mixed.  In such a block, we use default threads for TBB, and p/2 threads for
/// OpenMP.  If GTSAM is not compiled to use both TBB and OpenMP, this has no
/// effect.
class TbbOpenMPMixedScope {
 ...
 public:
  ...
  TbbOpenMPMixedScope() : previousOpenMPThreads(omp_get_num_threads()) {
    omp_set_num_threads(omp_get_num_procs() / 4);
  }

  ~TbbOpenMPMixedScope() { omp_set_num_threads(previousOpenMPThreads); }
#else
 ...
 *
 * This is required to make boost::make_shared and etc respect alignment, which
 * is essential for the Python wrappers to work properly.
 *
 * Explanation
 * =============
 * When a GTSAM type is not declared with the type alias
 * `_eigen_aligned_allocator_trait = void`, the first template will be taken so
 * `needs_eigen_aligned_allocator` will be resolved to `std::false_type`.
 *
 * Otherwise, it will resolve to the second template, which will be resolved to
 * `std::true_type`.
 *
...
struct needs_eigen_aligned_allocator<
    T, void_t<typename T::_eigen_aligned_allocator_trait>> : std::true_type {};

}  // namespace gtsam

/**
 * This marks a GTSAM object to require alignment. With this macro an object
 * will automatically be allocated in aligned memory when one uses
 * `gtsam::make_shared`. It reduces future misalignment problems that is hard to
 * debug. See
