...
bool equal_with_abs_tol(const Eigen::DenseBase<MATRIX>& A,
 ...
 */
GTSAM_EXPORT bool assert_equal(const Matrix& A, const Matrix& B,
                               double tol = 1e-9);

/**
 * inequals with an tolerance, prints out message if within tolerance
 */
GTSAM_EXPORT bool assert_inequal(const Matrix& A, const Matrix& B,
                                 double tol = 1e-9);

/**
 * equals with an tolerance, prints out message if unequal
 */
GTSAM_EXPORT bool assert_equal(const std::list<Matrix>& As,
                               const std::list<Matrix>& Bs, double tol = 1e-9);

/**
 * check whether the rows of two matrices are linear independent
 */
GTSAM_EXPORT bool linear_independent(const Matrix& A, const Matrix& B,
                                     double tol = 1e-9);

/**
 * check whether the rows of two matrices are linear dependent
 */
GTSAM_EXPORT bool linear_dependent(const Matrix& A, const Matrix& B,
                                   double tol = 1e-9);
 ...
 */
GTSAM_EXPORT void save(const Matrix& A, const std::string& s,
 ...
 * dlmread command.
 */
GTSAM_EXPORT std::istream& operator>>(std::istream& inputStream,
 ...
 */
template <class MATRIX>
Eigen::Block<const MATRIX> sub(const MATRIX& A, size_t i1, size_t i2, size_t j1,
                               size_t j2) {
...
void zeroBelowDiagonal(MATRIX& A, size_t cols = 0) {
  ...
  for (size_t j = 0; j < k; ++j) A.col(j).segment(j + 1, m - (j + 1)).setZero();
}
...
struct Reshape {
  // TODO replace this with Eigen's reshape function as soon as available.
  // (There is a PR already pending :
  // https://bitbucket.org/eigen/eigen/pull-request/41/reshape/diff)
  typedef Eigen::Map<const Eigen::Matrix<double, OutM, OutN, OutOptions> >
      ReshapedType;
  static inline ReshapedType reshape(
      const Eigen::Matrix<double, InM, InN, InOptions>& in) {
    return in.data();
  }
};

/// Reshape specialization that does nothing as shape stays the same (needed to
/// not be ambiguous for square input equals square output)
...
struct Reshape<N, M, InOptions, M, N, InOptions> {
  typedef
      typename Eigen::Matrix<double, M, N, InOptions>::ConstTransposeReturnType
          ReshapedType;
  static inline ReshapedType reshape(
      const Eigen::Matrix<double, M, N, InOptions>& in) {
    return in.transpose();
...
template <int OutM, int OutN, int OutOptions, int InM, int InN, int InOptions>
inline
    typename Reshape<OutM, OutN, OutOptions, InM, InN, InOptions>::ReshapedType
    reshape(const Eigen::Matrix<double, InM, InN, InOptions>& m) {
...
/** Return the inverse of a S.P.D. matrix.  Inversion is done via Cholesky
 * decomposition. */
 ...
 * You can just pass empty matrices U,V, and vector S, they will be
 * re-allocated.
 */
 ...
 */
GTSAM_EXPORT boost::tuple<int, double, Vector> DLT(const Matrix& A,
...
std::string formatMatrixIndented(const std::string& label, const Matrix& matrix,
                                 bool makeVectorHorizontal = false);
...
inline void multiplyAdd(double alpha, const Matrix& A, const Vector& x,
                        Vector& e) {
  e += alpha * A * x;
}
inline void multiplyAdd(const Matrix& A, const Vector& x, Vector& e) {
  e += A * x;
}
inline void transposeMultiplyAdd(double alpha, const Matrix& A, const Vector& e,
                                 Vector& x) {
  x += alpha * A.transpose() * e;
}
inline void transposeMultiplyAdd(const Matrix& A, const Vector& e, Vector& x) {
  x += A.transpose() * e;
}
inline void transposeMultiplyAdd(double alpha, const Matrix& A, const Vector& e,
                                 SubVector x) {
...
inline void insertColumn(Matrix& A, const Vector& col, size_t j) {
  A.col(j) = col;
}
inline void insertColumn(Matrix& A, const Vector& col, size_t i, size_t j) {
