...
ScalarExpressionInequalityConstraint::GeqZero(const Double_& expression,
  ...
  return std::make_shared<ScalarExpressionInequalityConstraint>(neg_expr,
                                                                sigma);
}
...
ScalarExpressionInequalityConstraint::LeqZero(const Double_& expression,
                                              const double& sigma) {
  return std::make_shared<ScalarExpressionInequalityConstraint>(expression,
                                                                sigma);
}
...
ScalarExpressionInequalityConstraint::penaltyFactor(const double mu) const {
  ...
  return std::make_shared<ExpressionFactor<double>>(penaltyNoise(mu), 0.0,
                                                    penalty_expression);
}
