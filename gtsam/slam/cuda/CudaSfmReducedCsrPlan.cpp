...
std::uint64_t CudaSfmReducedCsrPlan::PairKey(int first, int second) {
  return (static_cast<std::uint64_t>(static_cast<std::uint32_t>(first)) << 32) |
         static_cast<std::uint32_t>(second);
}

std::uint64_t CudaSfmReducedCsrPlan::ScalarKey(int row, int column) {
  return PairKey(row, column);
}

CudaSfmReducedCsrPlan::CudaSfmReducedCsrPlan(
...
bool CudaSfmReducedCsrPlan::hasCameraPair(int cameraI, int cameraJ) const {
...
int CudaSfmReducedCsrPlan::valueOffset(int cameraI, int cameraJ, int localRow,
...
Ordering CudaSfmReducedCsrPlan::colamdOrdering() const {
