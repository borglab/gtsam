...
__global__ void accumulateRhsAndBlocksKernel(
    const SfmObservation* observations, size_t observationCount,
...
struct SfmFullNormalProblem::Impl {
  class FullNormalOperator final : public LinearOperator {
   public:
    explicit FullNormalOperator(const Impl* owner) : owner_(owner) {}
  ...
  class FullNormalPreconditioner final : public Preconditioner {
   public:
    explicit FullNormalPreconditioner(const Impl* owner) : owner_(owner) {}
    int dimension() const override { return owner_->dimension; }
    void apply(const double* input, double* output,
               cudaStream_t stream) const override {
      if (!owner_->prepared) {
  ...
  const SfmProjectionLinearization* linearization = nullptr;
  int numCameras = 0;
  int numPoints = 0;
  int dimension = 0;
  double lambda = 0.0;
  ...
  DeviceArray<double> cameraBlocks;
  DeviceArray<double> pointBlocks;
  DeviceArray<double> cameraFactors;
  DeviceArray<double> pointFactors;
  DeviceSparseSpdSystem sparseSystem;
  FullNormalOperator linearOperator;
  FullNormalPreconditioner preconditioner;

  void setShape(const SfmProjectionBatch& newBatch, int newNumCameras) {
    if (newNumCameras < 0 ||
...
SfmFullNormalProblem::SfmFullNormalProblem()
    : impl_(std::make_unique<Impl>()) {}
SfmFullNormalProblem::~SfmFullNormalProblem() = default;
SfmFullNormalProblem::SfmFullNormalProblem(
    SfmFullNormalProblem&&) noexcept = default;
SfmFullNormalProblem& SfmFullNormalProblem::operator=(
    SfmFullNormalProblem&&) noexcept = default;

void SfmFullNormalProblem::initialize(
    const SfmProjectionBatch& batch, int numCameras, cudaStream_t) {
...
void SfmFullNormalProblem::initializeSparse(
    ...
    DeviceTransferSummary* transferProfile) {
  impl_->setShape(batch, numCameras);
...
void SfmFullNormalProblem::linearize(
  ...
  if (impl_->explicitSystemInitialized) {
    accumulateSfmNormalEquations(linearization, *impl_->batch,
                                     impl_->numCameras,
...
SfmFullNormalView prepareFullNormal(
    ImplType* impl, double lambda,
    const DeviceArray<double>* dampingDiagonal, cudaStream_t stream) {
...
SfmFullNormalView SfmFullNormalProblem::prepare(
...
DeviceSparseSpdSystem& SfmFullNormalProblem::prepareSparse(
    double lambda, cudaStream_t stream) {
...
int SfmFullNormalProblem::dimension() const { return impl_->dimension; }
size_t SfmFullNormalProblem::linearizationCount() const {
  return impl_->linearizationCount;
}
size_t SfmFullNormalProblem::preparationCount() const {
