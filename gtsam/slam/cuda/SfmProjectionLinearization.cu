...
template <bool kWhitened, bool kRobust>
__global__ void accumulateSfmNormalEquationsKernel(
    const DevicePinholeCameraCal3Bundler* cameras, const DevicePoint3* points,
    const SfmObservation* observations, size_t numObservations,
    const SfmSqrtInfo2* sqrtInfos,
    const SfmRobustModel* robustModels,
...
__global__ void accumulateLinearizedSfmNormalEquationsKernel(
    const SfmObservation* observations, size_t numObservations,
...
void accumulateSfmNormalEquations(
  ...
  if (numCameras < 0) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations requires nonnegative camera count");
  }
  if (static_cast<size_t>(numCameras) < batch.numCameras()) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations camera count smaller than batch");
  }
  ...
  if (batch.numCameras() > cameraBlock.values.size()) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations camera batch/value size mismatch");
  }
  if (batch.numPoints() > pointBlock.values.size()) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations point batch/value size mismatch");
  }
  ...
  if (static_cast<size_t>(numCameras) > maxInt / 9 ||
    ...
    throw std::invalid_argument(
        "accumulateSfmNormalEquations dimension exceeds int range");
  }
  const size_t requiredRows =
      9 * static_cast<size_t>(numCameras) + 3 * batch.numPoints();
  if (system->rows() != static_cast<int>(requiredRows)) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations system row count mismatch");
  }
  ...
  if (gridSizeSize > static_cast<size_t>(std::numeric_limits<int>::max())) {
    throw std::invalid_argument(
        "accumulateSfmNormalEquations grid size exceeds CUDA launch limit");
  }
  ...
  DeviceArray<int> missingEntries(1);
  missingEntries.zero(stream);

  const int gridSize = static_cast<int>(gridSizeSize);
  const SfmSqrtInfo2* sqrtInfos = sqrtInfoDataOrNull(
      batch, numObservations, "accumulateSfmNormalEquations");
  const SfmRobustModel* robustModels = robustModelDataOrNull(
      batch, numObservations, "accumulateSfmNormalEquations");
  if (batch.noiseMode() == SfmProjectionNoiseMode::Robust) {
    accumulateSfmNormalEquationsKernel<true, true>
        <<<gridSize, kProjectionLinearizationBlockSize, 0, stream>>>(
            cameraBlock.values.data(), pointBlock.values.data(),
            batch.observations().data(), numObservations, sqrtInfos,
    ...
    accumulateSfmNormalEquationsKernel<true, false>
        <<<gridSize, kProjectionLinearizationBlockSize, 0, stream>>>(
            cameraBlock.values.data(), pointBlock.values.data(),
            batch.observations().data(), numObservations, sqrtInfos,
    ...
    accumulateSfmNormalEquationsKernel<false, false>
        <<<gridSize, kProjectionLinearizationBlockSize, 0, stream>>>(
            cameraBlock.values.data(), pointBlock.values.data(),
  ...
  if (!hostMissingEntries.empty() && hostMissingEntries[0] != 0) {
    throw std::runtime_error(
        "accumulateSfmNormalEquations missing CSR entries");
  }
}
...
void accumulateSfmNormalEquations(
  ...
  DeviceArray<int> missingEntries(1);
  missingEntries.zero(stream);
  accumulateLinearizedSfmNormalEquationsKernel
