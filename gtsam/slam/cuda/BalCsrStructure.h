...
class BalCsrStructure {
 public:
  static BalCsrStructure fromSfmData(const SfmData& data) {
    BalCsrStructure structure;
    ...
    if (structure.numCameras_ > kMaxInt / kCameraDim ||
      ...
      throw std::invalid_argument("BalCsrStructure dimension too large");
    }
    ...
    for (size_t cameraIndex = 0; cameraIndex < structure.numCameras_;
      ...
      for (int row = 0; row < kCameraDim; ++row) {
        ...
        if (nnz > kMaxInt) {
          throw std::invalid_argument("BalCsrStructure too many nonzeros");
        }
    ...
    if (offset != structure.colIndices_.size()) {
      throw std::logic_error("BalCsrStructure fill mismatch");
    }
      ...
      for (const SfmMeasurement& measurement : track.measurements) {
        if (measurement.first >= numCameras_) {
          throw std::invalid_argument(
              "BalCsrStructure camera index out of range");
        }
