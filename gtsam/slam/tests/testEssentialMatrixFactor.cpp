...
TEST(ThreeViewEssentialMatrixFactor, evaluateErrorJacobians) {
  Key key_A_E_B(1);
  Key key_A_E_C(2);
  // initialize essential matrix
  ...
  val.insert(key_A_E_C, A_E_C);

  Vector3 vA(1.0, 2.0, 1.0);
...
TEST(ThreeViewEssentialMatrixFactor, minimization) {
  ...
  // Check result
  EXPECT(assert_equal(result.at<EssentialMatrix>(key_i1Ei2), i1Ei2_gt, 1e-3));
  EXPECT(assert_equal(result.at<EssentialMatrix>(key_i1Ei3), i1Ei3_gt, 1e-3));
