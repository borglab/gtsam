...
TEST( testPoseRotationFactor, level3_zero_error ) {
  ...
  Matrix expH1 = numericalDerivative22<Vector3,Pose3RotationPrior,Pose3>(evalFactorError3, factor, pose1, GTSAM_UNITTEST_TOLERANCE_LOOSE);
...
TEST( testPoseRotationFactor, level2_zero_error ) {
  ...
  Matrix expH1 = numericalDerivative22<Vector1,Pose2RotationPrior,Pose2>(evalFactorError2, factor, pose1, GTSAM_UNITTEST_TOLERANCE_LOOSE);
  EXPECT(assert_equal(expH1, actH1, tol));
}
