...
-   [SfmLevenbergMarquardtOptimizer](doc/SfmLevenbergMarquardtOptimizer.ipynb) : Fully GPU-resident Levenberg-Marquardt for BAL-style bundle adjustment (`GeneralSFMFactor<PinholeCamera<Cal3Bundler>, Point3>` graphs); dense-Schur or cuDSS linear solvers.
-   [GNC with the CUDA SFM optimizer](doc/SfmGncOptimizer.ipynb) : Robust (outlier-rejecting) bundle adjustment using `GncOptimizer` with the CUDA optimizer as inner solver.
