...
/**
 * @file   SmartProjectionFactorBase.h
 * @brief  Base class for smart projection factors
 * @author Luca Carlone
 * @author Zsolt Kira
...
/**
 * Common base for monocular smart projection factors. It triangulates the
 * observed landmark, caches the result, and provides the shared error and
 * linearization implementations. Derived factors decide how their variables
 * are converted into cameras by overriding SmartFactorBase::cameras.
 */
template <class CAMERA>
class SmartProjectionFactorBase : public SmartFactorBase<CAMERA> {
 private:
  typedef SmartFactorBase<CAMERA> Base;
  typedef SmartProjectionFactorBase<CAMERA> This;
 ...
 public:
  ...
  TriangulationResult triangulateSafe(const Cameras& cameras) const {
    size_t m = cameras.size();
    if (m < 2)  // if we have a single pose the corresponding factor is
                // uninformative
  ...
  createRegularImplicitSchurFactor(const Cameras& cameras,
                                   double _lambda) const {
  ...
  virtual std::shared_ptr<RegularImplicitSchurFactor<CAMERA>>
  linearizeToImplicit(const Values& values, double _lambda = 0.0) const {
   ...
   * @param values Values structure which must contain camera poses for this
   * factor
   * @return a Gaussian factor
   */
  std::shared_ptr<GaussianFactor> linearizeDamped(
      const Cameras& cameras, const double _lambda = 0.0) const {
   ...
   */
  std::shared_ptr<GaussianFactor> linearizeDamped(
      const Values& values, const double _lambda = 0.0) const {
    // depending on flag set on construction we may linearize to different
    // linear factors
  ...
  bool triangulateAndComputeJacobians(typename Base::FBlocks& Fs, Matrix& E,
    ...
    Cameras cameras = this->cameras(values);
    bool nonDegenerate = triangulateForLinearize(cameras);
    if (nonDegenerate) computeJacobiansWithTriangulatedPoint(Fs, E, b, cameras);
    return nonDegenerate;
  }

  /// takes values
  bool triangulateAndComputeJacobiansSVD(typename Base::FBlocks& Fs,
                                         Matrix& Enull, Vector& b,
  ...
  double totalReprojectionError(
      const Cameras& cameras, std::optional<Point3> externalPoint = {}) const {
    if (externalPoint)
    ...
    if (result_)
      ...
      Unit3 backprojected =
          cameras.front().backprojectPointAtInfinity(this->measured_.at(0));
