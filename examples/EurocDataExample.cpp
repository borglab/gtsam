 ...
 * This version is refactored to support multiple IMU preintegration techniques.
 * @author Matt Kielo, Porter Zach
 * @modifier AI Assistant (Refactoring and Enhancements for Multiple Techniques)
 */
 ...
 #include <gtsam/navigation/PreintegratedRotation.h> // For PreintegratedRotationParams definition
 ...
 // --- Configuration ---
 const string base_euroc_data_path = "C:/Users/porte/Desktop/projects/works25/euroc/data"; // PLEASE UPDATE THIS PATH
 ...
 // --- Data Structures (condensed for brevity, no changes in structure) ---
 ...
 struct AggregatedRunResults {
     ...
     void print_summary() const { /* ... (implementation as before) ... */ 
 ...
 // --- Helper Functions (condensed, no changes in logic) ---
 ...
 public:
     string name() const override { return "CombinedIMU (Fornasier et al. style)"; }
 
     shared_ptr<PreintegrationParams> createPreintegrationParams(
         const SensorParams& sensor_params, const Vector3& gravity_n_vec,
         double integration_sigma, const Vector3& body_omega_coriolis /* body_omega_coriolis effectively unused for this technique to match original */) const override {
         ...
         // MATCH ORIGINAL SCRIPT'S n_gravity SETTING:
         // Original script assigned the potentially non-unit gravity_n_vec to p_combined->n_gravity
         // after MakeSharedU (which sets n_gravity to default unit vector and g to norm).
         p_combined->n_gravity = gravity_n_vec; 
         
         // MATCH ORIGINAL SCRIPT'S COVARIANCE SETTINGS:
         // (and ensure body_P_sensor and omegaCoriolis remain default/unset on p_combined,
         //  as original script did not explicitly set these on the PreintegrationParams object)
         double nominal_imu_dt = 1.0 / sensor_params.rate_hz;
         if (nominal_imu_dt <= 1e-9) nominal_imu_dt = 0.005; // Defensive, as in original
         ...
         p_combined->biasAccOmegaInt = I_6x6 * 1e-5; // Default, as in original script
     ...
     IntervalEvaluationResult evaluateInterval(
         ...
         for (size_t k = imu_idx_start_global; k < imu_idx_end_global; ++k) {
             ...
             // MATCHING ORIGINAL SCRIPT'S TIMESTAMP CHECK (stricter lower bound)
             if (imu.timestamp < gt_i.timestamp || imu.timestamp > gt_j.timestamp + 1e-9) {
                 if (imu.timestamp < previous_imu_t && imu.timestamp >= gt_i.timestamp) previous_imu_t = imu.timestamp; // Match original condition
                 continue;
             }
             double current_imu_t = imu.timestamp; double dt = current_imu_t - previous_imu_t;
             if (dt < 1e-9) { // Effectively zero dt or out of order
                 if (dt < 0 && abs(dt) > 1e-7) { /* cerr << "Warning: Negative IMU dt " << dt << endl; */ } // Logging diff is ok
         ...
         // MATCH ORIGINAL SCRIPT'S BIAS ERROR LOGIC
 ...
 public:
     string name() const override { return "ManifoldIMU (Forster et al. style)"; }
     ...
     shared_ptr<PreintegrationParams> createPreintegrationParams(
         ...
         if (gravity_n_vec.norm() > 1e-9) {
             p_manifold->n_gravity = gravity_n_vec.normalized();
         } else {
             p_manifold->n_gravity = Vector3(0,0,-1);
         }
     ...
     IntervalEvaluationResult evaluateInterval(
         ...
         for (size_t k = imu_idx_start_global; k < imu_idx_end_global; ++k) {
             const ImuData& imu = all_imu_data[k];
              if (imu.timestamp < gt_i.timestamp -1e-9 || imu.timestamp > gt_j.timestamp + 1e-9) { // Original range slightly different, but this is for Manifold
                 if (imu.timestamp < previous_imu_t && imu.timestamp >= gt_i.timestamp-1e-9) previous_imu_t = imu.timestamp;
                 continue;
             }
             double current_imu_t = imu.timestamp; double dt = current_imu_t - previous_imu_t;
             if (dt < 1e-9) {
 ...
 // --- Results Aggregation and Printing (condensed, no changes in logic) ---
 ...
 int main(int argc, char* argv[]) {
     ...
     for (const auto& tech_ptr : techniques) {
         ...
         for (const string& dataset_name : dataset_sequences) {
             ...
             // Print sensor params only once per dataset (e.g., for the first technique)
