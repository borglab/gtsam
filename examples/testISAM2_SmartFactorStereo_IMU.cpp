/**
 * @file testISAM2_SmartFactorStereo_IMU.cpp
...
int main(int argc, char* argv[]) {
  ...
  graph.emplace_shared<PriorFactor<Pose3>>(X(0), Pose3::identity(),
                                           priorPoseNoise);
  initialEstimate.insert(X(0), Pose3::identity());
  newTimestamps[X(0)] = 1.0;

  // Bias prior
  graph.add(PriorFactor<imuBias::ConstantBias>(B(0), imu.priorImuBias,
                                               imu.biasNoiseModel));
  initialEstimate.insert(B(0), imu.priorImuBias);
  newTimestamps[B(0)] = 1.0;

  // Velocity prior - assume stationary
  graph.add(
      PriorFactor<Vector3>(V(0), Vector3(0, 0, 0), imu.velocityNoiseModel));
  initialEstimate.insert(V(0), Vector3(0, 0, 0));
  newTimestamps[V(0)] = 1.0;
