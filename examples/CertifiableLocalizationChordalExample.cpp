...
/**
 * @file CertifiableLocalizationChordalExample.cpp
 * @brief Certifiable Pose3 localization solved with the chordal SDP.
...
int main() {
  ...
  const auto keyedRecovered_kTws =
      ExtractQcqpValues<Pose3, 1>(chordalSdp.qcqpValues());
  const std::vector<double> recoveredPoseEVRs = chordalSdp.variableEVRs();
